  ...
  try {
    ...
    const conversations: Conversation[] = data.conversations.map((serverConv: any) => {
      ...
      if (serverConv.type === 'group') {
        baseConv.groupName = serverConv.groupName || serverConv.name;
        baseConv.groupAvatar = serverConv.groupAvatar || serverConv.avatarUrl;
        baseConv.admins = serverConv.admins || [];
        baseConv.createdBy = serverConv.createdBy || '';
        
        console.log(`📋 Loaded group "${baseConv.groupName}": createdBy=${baseConv.createdBy}, admins=${JSON.stringify(baseConv.admins)}`);
      }
