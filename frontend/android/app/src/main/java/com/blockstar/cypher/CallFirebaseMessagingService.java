...
public class CallFirebaseMessagingService extends FirebaseMessagingService {
    ...
    private void createAllNotificationChannels() {
        ...
        Log.d(TAG, "  ✅ Created 'calls' channel");
        ...
        messagesChannel.setLightColor(0xFF10B981);
        ...
        Log.d(TAG, "  ✅ Created 'messages' channel");
        ...
        missedChannel.setLightColor(0xFFEF4444);
        ...
        Log.d(TAG, "  ✅ Created 'missed_calls' channel");
    ...
    private void handleIncomingCall(Map<String, String> data, RemoteMessage.Notification fcmNotification) {
        ...
        if (callerName == null || callerName.isEmpty()) {
            callerName = data.get("caller");
        }
        String callType = data.get("callType");
        
        // ════════════════════════════════════════════════════════════════
        // NEW: Extract auth token and deep link URL from notification
        // ════════════════════════════════════════════════════════════════
        String authToken = data.get("authToken");
        String callUrl = data.get("callUrl");
        ...
        Log.d(TAG, "  Has Auth Token: " + (authToken != null && !authToken.isEmpty()));
        Log.d(TAG, "  Has Call URL: " + (callUrl != null && !callUrl.isEmpty()));
        Log.d(TAG, "═══════════════════════════════════════");

        DebugLogger.logCall("INCOMING CALL (FCM)", callId, callerId, callerName, callType);

        // Cancel any existing FCM notification
        cancelFcmNotification(callId);
        DebugLogger.log("Cancelled FCM notification with tag: incoming-call-" + callId);

        // Store call data in SharedPreferences
        SharedPreferences prefs = getSharedPreferences(PREFS_NAME, MODE_PRIVATE);
        ...
        // ════════════════════════════════════════════════════════════════
        // Store auth token and URL for deep linking
        // ════════════════════════════════════════════════════════════════
        if (authToken != null && !authToken.isEmpty()) {
            editor.putString("pending_call_auth_token", authToken);
        }
        if (callUrl != null && !callUrl.isEmpty()) {
            editor.putString("pending_call_url", callUrl);
        }
        
        editor.apply();
        
        DebugLogger.log("Saved call data to SharedPreferences (with auth token)");
        ...
        // Pass auth token and URL to service
        if (authToken != null) {
            serviceIntent.putExtra("authToken", authToken);
        }
        if (callUrl != null) {
            serviceIntent.putExtra("callUrl", callUrl);
        }
